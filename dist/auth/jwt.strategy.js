...
let JwtStrategy = class JwtStrategy extends (0, passport_1.PassportStrategy)(passport_jwt_1.Strategy) {
    ...
    constructor(authService, prismaService) {
        super({
            ...
            secretOrKey: 'SECRET_KEY', // Debes cambiar 'SECRET_KEY' por tu propia clave secreta
        });
    ...
    async validate(payload) {
        ...
        const user = await this.prismaService.user.findUnique({
            where: { id: payload.sub }, // Validamos con el ID del usuario
        });
        if (!user) {
            throw new Error('Unauthorized');
        }
        return user; // Aquí devolvemos el usuario validado para que pueda acceder a la ruta
    }
};
